...
import { globalManifest } from "./manifest.mjs";
...
async function loadStack(functionsDir) {
	...
	for (const req of requiredAPIs) {
		requiresAPI(req.api, req.reason);
	}
	const existingExtensions = globalManifest.extensions && typeof globalManifest.extensions === "object" ? globalManifest.extensions : {};
		...
		if (Array.isArray(legacyParams)) {
			for (const p of legacyParams) {
				if (typeof p?.toSpec === "function") {
					...
					const manifestParams = globalManifest.params;
