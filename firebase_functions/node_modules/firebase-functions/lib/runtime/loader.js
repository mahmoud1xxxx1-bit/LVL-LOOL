...
const require_runtime_manifest = require('./manifest.js');
...
async function loadStack(functionsDir) {
	...
	for (const req of requiredAPIs) {
		require_common_api.requiresAPI(req.api, req.reason);
	}
	const existingExtensions = require_runtime_manifest.globalManifest.extensions && typeof require_runtime_manifest.globalManifest.extensions === "object" ? require_runtime_manifest.globalManifest.extensions : {};
	require_runtime_manifest.globalManifest.extensions = {
		...extensions,
		...existingExtensions
	};
	if (!require_runtime_manifest.globalManifest.requiredAPIs) {
		require_runtime_manifest.globalManifest.requiredAPIs = [];
	}
	const legacySymbols = Object.getOwnPropertySymbols(globalThis).filter((sym) => (sym.description || sym.toString()).includes("firebase-functions:params:declaredParams:v"));
	for (const sym of legacySymbols) {
		const legacyParams = globalThis[sym];
		if (Array.isArray(legacyParams)) {
			for (const p of legacyParams) {
				if (typeof p?.toSpec === "function") {
					const spec = p.toSpec();
					if (!Array.isArray(require_runtime_manifest.globalManifest.params)) {
						require_runtime_manifest.globalManifest.params = [];
					}
					const manifestParams = require_runtime_manifest.globalManifest.params;
	...
	const stack = {
		...
		endpoints,
		...require_runtime_manifest.globalManifest
	};
	return stack;
}
