...
function createSnapshot(event) {
	if (event.datacontenttype?.includes("application/protobuf") || Buffer.isBuffer(event.data)) {
		...
		return require_common_providers_firestore.createSnapshotFromJson(event.data, getPath(event), event.data?.value?.createTime, event.data?.value?.updateTime, event.database);
	} else {
...
function createBeforeSnapshot(event) {
	if (event.datacontenttype?.includes("application/protobuf") || Buffer.isBuffer(event.data)) {
		...
		return require_common_providers_firestore.createBeforeSnapshotFromJson(event.data, getPath(event), event.data?.oldValue?.createTime, event.data?.oldValue?.updateTime, event.database);
	} else {
		require_logger_index.error(`Cannot determine payload type, datacontenttype is ${event.datacontenttype}, failing out.`);
		throw Error("Error: Cannot parse event payload.");
	}
